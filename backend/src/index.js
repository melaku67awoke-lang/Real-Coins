...
async function handle(request, env) {
  ...
  if (request.method === 'POST' && path === '/v1/kyc') {
    ...
    const existing = await env.DB.prepare(`SELECT status FROM kyc_records WHERE admin_account_id = ?1 LIMIT 1`).bind(auth.account.id).first();
    ...
    const nowMs = Date.now();
    await env.DB.prepare(`INSERT INTO kyc_records(admin_account_id,full_name,id_type,id_number,front_id_url,back_id_url,status,submitted_at_ms,reviewed_at_ms,reviewed_by_admin_id,rejection_reason) VALUES (?1,?2,?3,?4,?5,?6,'PENDING',?7,NULL,NULL,NULL) ON CONFLICT(admin_account_id) DO UPDATE SET full_name=excluded.full_name,id_type=excluded.id_type,id_number=excluded.id_number,front_id_url=excluded.front_id_url,back_id_url=excluded.back_id_url,status='PENDING',submitted_at_ms=excluded.submitted_at_ms,reviewed_at_ms=NULL,reviewed_by_admin_id=NULL,rejection_reason=NULL`).bind(auth.account.id,fullName,idType,idNumber,frontIdUrl,backIdUrl,nowMs).run();
    const row = await env.DB.prepare(`SELECT admin_account_id AS userId,full_name AS fullName,id_type AS idType,id_number AS idNumber,front_id_url AS frontIdUrl,back_id_url AS backIdUrl,status,submitted_at_ms AS submittedAt,reviewed_at_ms AS reviewedAt,reviewed_by_admin_id AS reviewedByAdminId,rejection_reason AS rejectionReason FROM kyc_records WHERE admin_account_id=?1`).bind(auth.account.id).first();
    return json({ok:true,kyc:row},201);
  }
  if (request.method === 'GET' && path === '/v1/kyc/me') {
    const auth = await requireAuth(request, env); if (auth.response) return auth.response;
    const row = await env.DB.prepare(`SELECT admin_account_id AS userId,full_name AS fullName,id_type AS idType,id_number AS idNumber,front_id_url AS frontIdUrl,back_id_url AS backIdUrl,status,submitted_at_ms AS submittedAt,reviewed_at_ms AS reviewedAt,reviewed_by_admin_id AS reviewedByAdminId,rejection_reason AS rejectionReason FROM kyc_records WHERE admin_account_id=?1 LIMIT 1`).bind(auth.account.id).first();
  ...
  if (request.method === 'GET' && path === '/v1/admin/kyc/pending') {
    ...
    const rows = await env.DB.prepare(`SELECT admin_account_id AS userId,full_name AS fullName,id_type AS idType,id_number AS idNumber,front_id_url AS frontIdUrl,back_id_url AS backIdUrl,status,submitted_at_ms AS submittedAt,reviewed_at_ms AS reviewedAt,reviewed_by_admin_id AS reviewedByAdminId,rejection_reason AS rejectionReason FROM kyc_records WHERE status='PENDING' ORDER BY submitted_at_ms ASC`).all();
  ...
  if (request.method === 'POST' && kycReviewMatch) {
    ...
    const existing=await env.DB.prepare(`SELECT status FROM kyc_records WHERE admin_account_id=?1 LIMIT 1`).bind(targetUserId).first();
    ...
    await env.DB.batch([
      env.DB.prepare(`UPDATE kyc_records SET status=?1,reviewed_at_ms=?2,reviewed_by_admin_id=?3,rejection_reason=?4 WHERE admin_account_id=?5 AND status='PENDING'`).bind(newStatus,nowMs,auth.account.id,approve?null:reason,targetUserId),
    ...
    const row=await env.DB.prepare(`SELECT admin_account_id AS userId,full_name AS fullName,id_type AS idType,id_number AS idNumber,front_id_url AS frontIdUrl,back_id_url AS backIdUrl,status,submitted_at_ms AS submittedAt,reviewed_at_ms AS reviewedAt,reviewed_by_admin_id AS reviewedByAdminId,rejection_reason AS rejectionReason FROM kyc_records WHERE admin_account_id=?1`).bind(targetUserId).first();
  ...
  if(request.method==='GET'&&kycDocMatch){
    ...
    const row=await env.DB.prepare(`SELECT ${column} AS url FROM kyc_records WHERE admin_account_id=?1 LIMIT 1`).bind(target).first();
        ...
        WHERE a.id = ?1
          AND (
            ...
            OR (
              ...
              AND EXISTS (SELECT 1 FROM kyc_records k WHERE k.admin_account_id = a.owner_account_id AND (k.front_id_url = ?3 OR k.back_id_url = ?3))
            )
            OR EXISTS (
              SELECT 1
                FROM p2p_orders o
