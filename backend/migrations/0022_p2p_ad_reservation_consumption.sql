-- Extend SELL-ad reservation accounting with per-order consumption records.
-- Migration 0021 is already deployed and is intentionally immutable, so this
-- migration rebuilds the small ledger table with the additional event type.

PRAGMA foreign_keys=OFF;

-- The triggers created by migration 0021 reference the old ledger table.
-- Remove them before replacing that table so SQLite never has a dangling
-- trigger reference during the table rebuild.

DROP TRIGGER IF EXISTS trg_p2p_ad_reservation_insert_ledger;
DROP TRIGGER IF EXISTS trg_p2p_ad_reservation_delete_ledger;
DROP TRIGGER IF EXISTS trg_p2p_ad_reservation_ledger_immutable_update;
DROP TRIGGER IF EXISTS trg_p2p_ad_reservation_ledger_immutable_delete;

CREATE TABLE p2p_ad_reservation_ledger_v2 (
    id TEXT PRIMARY KEY,
    ad_id TEXT NOT NULL,
    account_id TEXT NOT NULL,
    order_id TEXT,
    event TEXT NOT NULL CHECK (
        event IN ('RESERVE', 'RELEASE', 'CONSUME')
    ),
...
INSERT INTO p2p_ad_reservation_ledger_v2(
    id,
    ad_id,
    account_id,
    order_id,
...
ALTER TABLE p2p_ad_reservation_ledger_v2
RENAME TO p2p_ad_reservation_ledger;
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_p2p_ad_reservation_reserve
    ON p2p_ad_reservation_ledger(ad_id)
    WHERE event = 'RESERVE';
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_p2p_ad_reservation_order_consume
    ON p2p_ad_reservation_ledger(ad_id, order_id)
    WHERE event = 'CONSUME';
...
BEGIN
    SELECT RAISE(
        ABORT,
        'p2p_ad_reservation_ledger_immutable'
    );
END;
