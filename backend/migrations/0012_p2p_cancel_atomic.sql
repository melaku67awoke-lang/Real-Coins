...
WHEN OLD.status = 'ESCROW_LOCKED' AND NEW.status = 'CANCELLED'
BEGIN
    SELECT CASE
    ...
    SELECT CASE
        WHEN (SELECT COUNT(*) FROM p2p_ads a
              WHERE a.id = OLD.ad_id) = 0
        THEN RAISE(ABORT, 'p2p_ad_not_found')
    END;

    SELECT CASE
        WHEN (SELECT type FROM p2p_ads WHERE id = OLD.ad_id) = 'BUY'
         AND (SELECT COUNT(*) FROM p2p_wallets w
...
BEGIN
    UPDATE p2p_wallets
       SET real_locked_balance = real_locked_balance - OLD.crypto_amount,
           updated_at_ms = COALESCE(NEW.completed_at_ms, OLD.expires_at_ms)
...
BEGIN
    UPDATE p2p_ads
       SET crypto_amount = crypto_amount + OLD.crypto_amount,
           max_order_etb = original_max_order_etb,
           is_active = CASE
