...
// -------------------------------------------------------------
// BUILD COMPATIBILITY HELPERS
// These bridge the current ViewModel calls to the repository API
// already present in this project. No P2P or unrelated behavior
// is changed.
// -------------------------------------------------------------

private suspend fun RealCoinRepository.syncKycFromBackendCompat(
    userId: String
): Result<KycEntity?> =
    runCatching {
...
private suspend fun RealCoinRepository.submitKycCompat(
    userId: String,
    fullName: String,
    idType: String,
    idNumber: String,
...
class RealCoinViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val userProfile: StateFlow<UserProfile> = combine(
        ...
        if (user == null) {
            ...
            UserProfile(
                ...
                realCoinBalance = (wallet?.realBalance ?: 0.0) + (wallet?.bonusRealBalance ?: 0.0),
    ...
    fun register(
        ...
        viewModelScope.launch {
            val result: Result<UserEntity> = repository.register(
                username,
                email,
                pass,
                referralCode
                    ...
                    .onSuccess {
                        ...
                        viewModelScope.launch {
                            ...
                            repository.getReferralSummary()
                                .onSuccess { referral ->
                                    _referralSummary.value = referral
                                }
                        }
    ...
    fun login(
        ...
        viewModelScope.launch {
            ...
            result.onSuccess { user ->
                ...
                /*
                 * NORMAL USERS:
                 * Refresh KYC from the authoritative backend before checking
                 * the gate. This is required when KYC was approved on another
                 * device: the local Room database on the login device may still
                 * contain NOT_SUBMITTED/PENDING even though the backend says
                 * VERIFIED.
                 */
                val syncedKyc =
                    repository.syncKycFromBackendCompat(user.id)

                if (syncedKyc.isFailure) {
                    repository.disconnectBackendSession()
                    onError(
                        syncedKyc.exceptionOrNull()?.message
                            ?: "Could not verify KYC status. Please try again."
                    )
                    return@onSuccess
                }

                val kycStatus =
                    syncedKyc.getOrNull()?.status
                        ?.trim()
                        ?.uppercase()
                        ?: "NOT_SUBMITTED"
    ...
    fun submitKyc(
        ...
        viewModelScope.launch {
            val result = repository.submitKycCompat(
                user.id,
                fullName,
                idType,
                idNumber,
