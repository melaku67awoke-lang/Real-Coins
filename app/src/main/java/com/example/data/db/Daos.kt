...
interface WalletDao {
    ...
    suspend fun deleteWalletByUserId(userId: String): Int
}
...
interface KycDao {
    ...
    @Query("DELETE FROM kyc_records WHERE userId = :userId")
    suspend fun deleteKycByUserId(userId: String): Int
...
interface RewardDao {
    ...
    @Query("SELECT * FROM reward_claims WHERE userId = :userId ORDER BY claimTimestamp DESC LIMIT 1")
    suspend fun getLatestClaimForUser(
        userId: String
    ): RewardClaimEntity?
