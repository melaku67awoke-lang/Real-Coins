...
@JsonClass(generateAdapter = true)
data class BackendAd(val id: String, val sellerId: String, val sellerName: String, val type: String, val cryptoAmount: Double, val fiatPrice: Double, val fiatOrderAmount: Double, val fiatCurrency: String, val paymentMethod: String, val paymentName: String, val accountNumber: String, val isActive: Int, val minOrderEtb: Double, val maxOrderEtb: Double, val originalMaxOrderEtb: Double, val createdAt: Long)
...
@JsonClass(generateAdapter = true)
data class KycResponse(val ok: Boolean, val kyc: BackendKyc?)
@JsonClass(generateAdapter = true)
data class KycListResponse(val ok: Boolean, val kycs: List<BackendKyc> = emptyList())
@JsonClass(generateAdapter = true)
data class KycReviewRequest(val approve: Boolean, val reason: String? = null)
@JsonClass(generateAdapter = true)
data class KycDocumentResponse(val ok: Boolean, val contentType: String?, val dataBase64: String?)
...
interface BackendApi {
    ...
    @GET("v1/kyc/me") suspend fun getMyKyc(@Header("Authorization") auth: String): Response<KycResponse>
    @GET("v1/admin/kyc/pending") suspend fun getPendingKyc(@Header("Authorization") auth: String): Response<KycListResponse>
    @POST("v1/admin/kyc/{userId}/review") suspend fun reviewKyc(@Header("Authorization") auth: String, @Path("userId") userId: String, @Body request: KycReviewRequest): Response<KycResponse>
    @GET("v1/admin/kyc/{userId}/document/{side}") suspend fun getKycDocument(@Header("Authorization") auth: String, @Path("userId") userId: String, @Path("side") side: String): Response<KycDocumentResponse>
