...
data class AdRequest(
    ...
    val minOrderEtb: Double,
    val maxOrderEtb: Double,
    val paymentMethod: String,
    val paymentName: String,
    val accountNumber: String,
    // Compatibility with deployed Worker versions that include the
    // total ETB value in an advertisement payload.
    val fiatOrderAmount: Double = cryptoAmount * fiatPrice
)
@JsonClass(generateAdapter = true)
...
data class BackendAd(
    ...
    val cryptoAmount: Double,
    val fiatPrice: Double,
    // Some deployed Worker versions return this field for ads. Keep it
    // optional/defaulted so a missing field can never break ad creation.
    val fiatOrderAmount: Double = cryptoAmount * fiatPrice,
    ...
    val minOrderEtb: Double,
    val maxOrderEtb: Double,
    val originalMaxOrderEtb: Double,
    val createdAt: Long
)
...
@JsonClass(generateAdapter = true)
data class BackendKyc(
    // New Worker versions return accountId; older deployed versions returned userId.
    // Keep both so the app can read either backend during the migration.
...
data class KycResponse(
    val ok: Boolean,
    val kyc: BackendKyc? = null
)

@JsonClass(generateAdapter = true)
data class AdminKycResponse(
    val ok: Boolean,
    val kycs: List<BackendKyc> = emptyList(),
...
interface BackendApi {
    ...
    @POST("v1/kyc") suspend fun submitKyc(@Header("Authorization") auth: String, @Body request: KycSubmitRequest): Response<KycResponse>
    @GET("v1/kyc") suspend fun myKyc(@Header("Authorization") auth: String): Response<KycResponse>
    // Compatibility with older Worker deployments.
    @GET("v1/kyc/me") suspend fun myKycLegacy(@Header("Authorization") auth: String): Response<KycResponse>
    @GET("v1/admin/kyc") suspend fun adminKycs(@Header("Authorization") auth: String): Response<AdminKycResponse>
    // Compatibility with older Worker deployments.
    @GET("v1/admin/kyc/pending") suspend fun adminKycsLegacy(@Header("Authorization") auth: String): Response<AdminKycResponse>
    @POST("v1/admin/kyc/{accountId}/review") suspend fun reviewKyc(@Header("Authorization") auth: String, @Path("accountId") accountId: String, @Body request: KycReviewRequest): Response<KycResponse>
    @GET("v1/admin/kyc/{accountId}/document/{side}") suspend fun adminKycDocument(@Header("Authorization") auth: String, @Path("accountId") accountId: String, @Path("side") side: String): Response<KycDocumentResponse>
