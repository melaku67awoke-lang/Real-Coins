...
class RealCoinRepository(private val db: AppDatabase, context: Context) {
    ...
    /**
     * Authenticates an already-existing account against the authoritative backend.
     *
     * IMPORTANT:
     * - Login NEVER registers an account as a fallback.
     * - The backend is authoritative for the authenticated account and role.
     * - This is required for fresh installations where Room is empty.
     */
    suspend fun connectBackendSession(
        ...
        return runCatching {
            var authResponse = backendApi.login(
                AuthRequest(
            ...
            /*
             * This method is also called immediately after a brand-new local
             * registration. In that registration path the backend account does
             * not exist yet, so the first backend login correctly returns 401.
             * Register the new account instead of showing the misleading
             * "Invalid email or password" message.
             *
             * Normal login is still protected: repository.login() performs the
             * backend authentication before this method is reached for an
             * existing account. Therefore this 401->register recovery is only
            ...
            if (
                ...
                authResponse = backendApi.register(
                    AuthRequest(
                ...
                // A 409 means the backend account was created already (for
                // example after a previous request completed). Recover by
                // authenticating it instead of leaving the local registration
                // unusable.
                if (
                    !authResponse.isSuccessful &&
                    authResponse.code() == 409
                ) {
                    authResponse = backendApi.login(
                        AuthRequest(
                            email = user.normalizedEmail,
                            password = password
                        )
            ...
            if (
                !authResponse.isSuccessful ||
                authResponse.body()?.sessionToken.isNullOrBlank()
            ) {
                throw backendFailure(
                    ...
                    when (authResponse.code()) {
                        ...
                        else -> "Backend account setup failed"
                    }
                )
            }
            ...
            val auth = authResponse.body()
                ?: throw IllegalStateException(
                    "Backend returned no authentication response"
                )
            ...
            // Do not trust the locally cached role. The backend account
            // returned after successful password authentication is authoritative.
            val backendRole = account.role
                ?.trim()
                ?.uppercase(Locale.ROOT)
                ?: "USER"

            if (backendRole != user.role.trim().uppercase(Locale.ROOT)) {
                throw SecurityException(
                    "Local account role does not match the authenticated backend role"
                )
            }
     ...
     * Login flow:
     *
     * 1. If Room has the account, authenticate the password against the
     *    authoritative backend first. This prevents stale local data from
     *    overriding the backend account/role.
     *
     * 2. If Room is empty (for example after a fresh installation), authenticate
     *    directly against the backend. A successful backend ADMIN account is
     *    mirrored locally only after the backend has verified the password.
     *
     * 3. A failed backend login NEVER creates/registers an account.
     */
    suspend fun login(
        ...
        if (localUser != null) {
            // Backend is authoritative. Do not accept a local password/role
            // without a successful backend authentication.
            val backendLogin = backendApi.login(
                AuthRequest(
                    email = localUser.normalizedEmail,
                    password = password
                )
            )

            if (
                !backendLogin.isSuccessful ||
                backendLogin.body()?.sessionToken.isNullOrBlank()
            ) {
                return Result.failure(
                    IllegalArgumentException(
                        if (backendLogin.code() == 401) {
                            "Invalid email or password"
                        } else {
                            "Backend login failed (HTTP ${backendLogin.code()})"
                        }
                    )
                )
            }

            val auth = backendLogin.body()
                ?: return Result.failure(
                    IllegalStateException("Backend returned no authentication response")
                )

            val account = auth.account
                ?: return Result.failure(
                    IllegalStateException("Backend returned no account")
                )
            ...
            if (backendRole != localUser.role.trim().uppercase(Locale.ROOT)) {
                return Result.failure(
                    SecurityException(
                        "Account role changed on the backend. Please update the local account."
                    )
                )
            }
            ...
            backendSession.token = auth.sessionToken
            backendSession.accountId = account.id
            backendSession.localUserId = localUser.id
        ...
        /*
         * Fresh-install path:
         * Room has no user, so local password verification is impossible.
         * Authenticate the supplied credentials directly against the backend.
         */
        val backendLogin = backendApi.login(
            AuthRequest(
                email = normalized,
            ...
            return Result.failure(
                IllegalArgumentException(
                    if (backendLogin.code() == 401) {
                        ...
                        "Account could not be authenticated (HTTP ${backendLogin.code()})"
                    }
                )
            )
        }
        ...
        val localUserId =
            "RC_BACKEND_${account.id}"

        val existingByBackendLocalId =
            userDao.getUserById(localUserId)

        val user = existingByBackendLocalId
            ?: run {
                val salt = PasswordHasher.generateSalt()
                ...
                db.withTransaction {
                    ...
                    // Normal users retain the existing KYC gate. ADMIN users
                    // receive a local placeholder only so existing UI flows
                    // that expect a KYC row cannot crash; ADMIN is never gated
                    // by KYC in RealCoinViewModel.
        ...
        if (user.role.trim().uppercase(Locale.ROOT) != backendRole) {
            return Result.failure(
                SecurityException(
                    "Authenticated backend role does not match the local account"
                )
            )
        }
    ...
    private fun mapBackendKyc(row: BackendKyc, localUserId: String = row.userId): KycEntity =
        KycEntity(
            userId = localUserId, fullName = row.fullName, idType = row.idType, idNumber = row.idNumber,
            documentAttached = !row.frontIdUrl.isNullOrBlank() && !row.backIdUrl.isNullOrBlank(),
            documentUri = row.frontIdUrl, frontIdUri = row.frontIdUrl, backIdUri = row.backIdUrl,
            status = row.status, submittedAt = row.submittedAt, reviewedAt = row.reviewedAt,
            reviewedByAdminId = row.reviewedByAdminId, rejectionReason = row.rejectionReason
        )

    suspend fun syncKycFromBackend(userId: String): Result<KycEntity?> = runCatching {
        val auth = backendAuth() ?: return@runCatching getKycForUserSync(userId)
        val response = backendApi.getMyKyc(auth)
        if (!response.isSuccessful) throw backendFailure(response.code(), "Could not sync KYC")
        val remote = response.body()?.kyc ?: return@runCatching getKycForUserSync(userId)
        val local = mapBackendKyc(remote, userId)
        kycDao.insertOrUpdateKyc(local)
        local
    }

    suspend fun submitKyc(
        userId: String, fullName: String, idType: String, idNumber: String,
        frontIdUri: String, backIdUri: String
    ): Result<KycEntity> {
        val user = userDao.getUserById(userId) ?: return Result.failure(IllegalStateException("User not found"))
        if (fullName.isBlank() || idType.isBlank() || idNumber.isBlank() || frontIdUri.isBlank() || backIdUri.isBlank()) {
            return Result.failure(IllegalArgumentException("ID type, ID number, and both ID documents are required"))
        }
        return runCatching {
            val auth = backendAuth() ?: throw SecurityException("Secure backend session is required for KYC")
            val frontUrl = uploadAttachmentIfNeeded(frontIdUri) ?: throw IllegalStateException("Could not upload front ID")
            val backUrl = uploadAttachmentIfNeeded(backIdUri) ?: throw IllegalStateException("Could not upload back ID")
            val response = backendApi.submitKyc(auth, KycSubmitRequest(fullName.trim(), idType.trim(), idNumber.trim(), frontUrl, backUrl))
            if (!response.isSuccessful) throw backendFailure(response.code(), "KYC submission failed")
            val remote = response.body()?.kyc ?: throw IllegalStateException("Backend returned no KYC record")
            val local = mapBackendKyc(remote, userId)
            kycDao.insertOrUpdateKyc(local)
            local
        }
    ...
    suspend fun getPendingKycRemote(): Result<List<KycEntity>> = runCatching {
        val auth = backendAuth() ?: throw SecurityException("Secure admin backend session is required")
        val response = backendApi.getPendingKyc(auth)
        if (!response.isSuccessful) throw backendFailure(response.code(), "Could not load pending KYC")
        response.body()?.kycs.orEmpty().map { mapBackendKyc(it) }
    }

    suspend fun reviewKyc(adminUserId: String, targetUserId: String, approve: Boolean, reason: String? = null): Result<KycEntity> = runCatching {
        val admin = userDao.getUserById(adminUserId)
        if (admin?.role?.uppercase(Locale.ROOT) != "ADMIN") throw SecurityException("Unauthorized: Only an authorized admin can review KYC")
        if (adminUserId == targetUserId) throw SecurityException("Security violation: Admin cannot review their own KYC")
        val auth = backendAuth() ?: throw SecurityException("Secure admin backend session is required")
        val response = backendApi.reviewKyc(auth, targetUserId, KycReviewRequest(approve, reason))
        if (!response.isSuccessful) throw backendFailure(response.code(), "KYC review failed")
        val remote = response.body()?.kyc ?: throw IllegalStateException("Backend returned no reviewed KYC")
        val local = mapBackendKyc(remote, targetUserId)
        kycDao.insertOrUpdateKyc(local)
        local
    }

    suspend fun openRemoteKycDocument(targetUserId: String, side: String): Result<String> = runCatching {
        val auth = backendAuth() ?: throw SecurityException("Secure admin backend session is required")
        val response = backendApi.getKycDocument(auth, targetUserId, side)
        if (!response.isSuccessful) throw backendFailure(response.code(), "Could not load KYC document")
        val body = response.body() ?: throw IllegalStateException("No KYC document returned")
        val uri = AttachmentStorage.saveImageBase64(appContext, body.dataBase64 ?: throw IllegalStateException("No KYC document data returned"), body.contentType ?: "image/jpeg", "kyc_$side")
            ?: throw IllegalStateException("Could not save KYC document")
        uri
    }
    ...
    companion object {
        ...
        // Daily reward uses the same 24-hour claim period as the free-spin
        // cooldown. Keep this public alias for the dashboard UI.
        const val DAILY_REWARD_COOLDOWN_MS =
            SPIN_COOLDOWN_MS
    ...
    suspend fun getDailyRewardState(
        userId: String
    ): DailyRewardState {
        // Daily Reward has its own state in reward_claims.
        // It must never read the spinning wheel's SpinStateEntity.
        val lastClaimAt =
            rewardDao.getLatestClaimForUser(userId)
                ?.claimTimestamp
        ...
        val nextClaimAt =
            if (lastClaimAt == null) {
                ...
                lastClaimAt + DAILY_REWARD_COOLDOWN_MS
            }
    ...
    suspend fun claimDailyReward(
        ...
        return db.withTransaction {
            ...
            // Daily Reward has an independent 24-hour cooldown.
            // This uses reward_claims, not SpinStateEntity, so claiming the
            // Daily Reward never consumes or resets the spinning wheel free spin.
            val lastClaimAt =
                rewardDao.getLatestClaimForUser(userId)
                    ?.claimTimestamp
            ...
            if (
                lastClaimAt != null &&
                now < lastClaimAt + DAILY_REWARD_COOLDOWN_MS
            ) {
                return@withTransaction Result.failure(
                    IllegalStateException(
                        "Daily reward is already claimed. Please wait until the next daily period."
                    )
                )
            }
            ...
            // Daily reward is fixed by the user's RewardPolicy level.
            // It is intentionally NOT the random spinning-wheel reward.
            val reward =
            ...
            if (
                ...
                return@withTransaction Result.failure(
                    IllegalStateException(
                        "Invalid daily reward configuration"
                    )
                )
            }

            // Daily Reward has its own cooldown/state.
            // It must NOT read or update SpinStateEntity, because the
            // spinning wheel has its own independent 24-hour free-spin
            // cooldown.

            // A valid normal user must always have a local Room wallet.
            // If an older installation or interrupted account-restore path
            // left the wallet row missing, initialize it here instead of
            // rejecting a legitimate daily reward claim. This does not
            // change an existing wallet or any existing balance.
            val wallet =
                walletDao.getWalletSync(userId)
                    ?: WalletEntity(
                        userId = userId,
            ...
            // Do NOT update SpinStateEntity here.
            // Daily Reward must not consume/reset the spinning wheel's
            // free-spin 24-hour cooldown.
            ...
            // Keep a durable claim record so the Daily Reward cooldown
            // survives app restarts and remains completely separate from
            ...
            ledgerDao.insertLedgerEntry(
                LedgerEntryEntity(
                    ...
                    before,
                    after,
                    "DAILY_REWARD_${UUID.randomUUID().toString().take(8)}",
                    "Fixed daily reward: ${rewardStatus.level.name}",
                    now
                )
            )
