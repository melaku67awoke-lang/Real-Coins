...
class RealCoinRepository(private val db: AppDatabase, context: Context) {
    ...
    suspend fun syncKycFromBackend(userId: String): Result<KycEntity?> {
        ...
        return runCatching {
            // The backend is authoritative for cross-device KYC. Never fall
            // back to the fresh-device NOT_SUBMITTED Room placeholder when
            // the remote request fails or returns no record.
            ...
            val remote = response.body()?.kyc
                ?: throw IllegalStateException(
                    "No KYC record was returned by the backend"
                )
