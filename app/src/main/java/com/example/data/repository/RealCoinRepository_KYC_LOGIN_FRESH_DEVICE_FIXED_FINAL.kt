...
class RealCoinRepository(private val db: AppDatabase, context: Context) {
    ...
    suspend fun syncKycFromBackend(userId: String): Result<KycEntity?> {
        val auth = backendAuth()
            ?: return Result.failure(SecurityException("Backend session is not connected"))

        return runCatching {
            // Always resolve the authenticated backend account first. This
            // prevents a stale local account/session from causing the KYC
            // gate to read the wrong identity after a fresh-device login.
            val me = backendApi.me(auth)
            if (!me.isSuccessful) {
                throw backendFailure(
                    me.code(),
            ...
            val account = me.body()?.account
                ?: throw IllegalStateException(
                    "Backend returned no authenticated account"
                )

            val sessionAccountId = backendSession.accountId
            if (!sessionAccountId.isNullOrBlank() && sessionAccountId != account.id) {
            ...
            val remote = response.body()?.kyc
                ?: throw IllegalStateException(
                    "No KYC record exists for this verified backend account"
                )
