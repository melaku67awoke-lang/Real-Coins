...
class RealCoinRepository(private val db: AppDatabase, context: Context) {
    ...
    private suspend fun ensureWalletExists(userId: String) {
        if (walletDao.getWalletSync(userId) == null) {
            walletDao.insertWallet(
                WalletEntity(
                    userId = userId,
                    realBalance = 0.0,
                    usdtBalance = 0.0,
                    bonusRealBalance = 0.0,
    ...
    /**
     * Creates the newly registered account on the authoritative backend and
     * establishes the returned authenticated session. Registration never uses
     * login as a substitute for account creation.
     */
    suspend fun registerBackendAccount(
        user: UserEntity,
        password: String,
        ...
        return runCatching {
            ...
            if (
                ...
                response.body()?.sessionToken.isNullOrBlank()
            ) {
                throw backendFailure(
                    response.code(),
                    when (response.code()) {
                        409 -> "Username or email is already registered"
                        400 -> "Registration data was rejected by the backend"
                        else -> "Backend registration failed"
                    }
                )
            }
            ...
            val auth = response.body()
                ?: throw IllegalStateException(
                    "Backend returned no registration response"
                )

            val account = auth.account
                ?: throw IllegalStateException(
                    "Backend returned no registered account"
                )
            ...
            if (backendRole != "USER") {
                throw SecurityException(
                    "Unexpected backend role returned during user registration"
                )
            }

            if (account.email.trim().lowercase(Locale.ROOT) != user.normalizedEmail) {
                throw SecurityException(
                    "Backend registration email does not match the requested account"
                )
            }
    ...
    suspend fun login(
        ...
        if (localUser != null) {
            ...
            ensureWalletExists(localUser.id)
    ...
    suspend fun submitKyc(
        ...
        if (
            ...
            idNumber.isBlank() ||
            !documentAttached ||
            frontIdUri.isNullOrBlank() ||
            backIdUri.isNullOrBlank()
        ) {
            return Result.failure(
                IllegalArgumentException(
                    "Full name, ID type, ID number, front ID and back ID are required"
                )
            )
        }
    ...
    suspend fun getDailyRewardState(
        ...
        val lastClaimAt =
            rewardDao.getLatestClaimForUser(userId)?.claimTimestamp
    ...
    suspend fun canClaimRewardToday(
        ...
        return System.currentTimeMillis() >=
            getDailyRewardState(userId).nextClaimAt
    }
    ...
    suspend fun getRewardStatus(
        ...
        val totalDepositUsd =
            depositDao.getConfirmedDepositTotal(userId)

        val level =
            RewardPolicy.levelFor(totalDepositUsd)

        val (next, progress) =
            RewardPolicy.progress(totalDepositUsd)
        ...
        val cycle =
            RewardPolicy.rewardCycleState(
                userId = userId,
                claims = rewardDao.getClaimsForUser(userId),
                orders = p2pDao.getOrdersForUserSync(userId)
            )
        ...
        return RewardStatus(
            ...
            qualifyingBuyCompleted = cycle.qualifyingBuyCompleted,
            qualifyingSellCompleted = cycle.qualifyingSellCompleted,
            activityRequirementMet = cycle.rewardActive,
            rewardActive = cycle.rewardActive,
    ...
    suspend fun claimDailyReward(
        ...
        return db.withTransaction {
            ...
            // Daily reward and free spin use independent cooldowns.
            // Do not update spin state when claiming the dashboard reward.
            walletDao.updateWallet(
                wallet.copy(
                    realBalance = after,
                    updatedAt = now
    ...
    suspend fun deleteSavedPaymentAccount(
        ...
        if (hasActiveP2POrder(userId)) {
            return Result.failure(
                IllegalStateException(
                    "Payment account cannot be deleted while you have an active P2P order"
                )
            )
        }
    ...
    private suspend fun hasActiveP2POrder(userId: String): Boolean {
        refreshP2PFromBackend()
        return p2pDao.getOrdersForUserSync(userId).any { order ->
            order.status == "ESCROW_LOCKED" ||
                order.status == "PAID" ||
    ...
    suspend fun deleteOwnP2PAd(
        ...
        if (hasActiveP2POrder(userId)) {
            return Result.failure(
                IllegalStateException(
                    "P2P ad cannot be deleted while you have an active P2P order"
                )
            )
        }
            ...
            return runCatching {
                ...
                if (ad.sellerId == buyerId) {
                    throw IllegalArgumentException(
                        "You cannot trade with your own P2P advertisement."
                    )
                }
