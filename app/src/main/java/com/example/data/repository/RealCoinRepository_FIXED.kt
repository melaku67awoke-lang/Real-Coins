...
class RealCoinRepository(private val db: AppDatabase, context: Context) {
    ...
    suspend fun submitKyc(
        ...
        // The current KYC UI supplies ID type plus separate front/back
        // documents. Keep the older repository call compatible for existing
        // tests/data while storing the new fields whenever they are supplied.
        if (idType.isNotBlank() && (frontIdUri.isNullOrBlank() || backIdUri.isNullOrBlank())) {
            return Result.failure(
                IllegalArgumentException(
                    "Both front and back ID documents are required"
                )
            )
        }
        ...
        return db.withTransaction {
            ...
            val updatedKyc = KycEntity(
                ...
                // Keep documentUri populated for compatibility with older code,
                // while the authoritative current fields are frontIdUri/backIdUri.
                documentUri = frontIdUri ?: documentUri,
