...
class RealCoinRepository(private val db: AppDatabase, context: Context) {
    ...
    suspend fun connectBackendSession(
        ...
        return runCatching {
            ...
            val auth = if (
                ...
                (login.code() == 401 || login.code() == 404) &&
                user.role != "ADMIN"
            ) {
                // Existing normal users may need their local account linked
                // to the backend account. NEVER auto-register an ADMIN as USER.
            ...
            if (user.role == "ADMIN" && account.role != "ADMIN") {
                throw SecurityException(
                    "Backend account is not authorized as ADMIN"
                )
            }
    ...
    private fun mapAd(ad: BackendAd): P2PAdEntity =
        P2PAdEntity(
            ...
            fiatOrderAmount = ad.fiatOrderAmount,
    ...
    suspend fun login(
        ...
        // First use the local account when it exists. This preserves the
        // existing local-login behavior.
        val localUser =
        ...
        if (localUser != null) {
            ...
            // Existing local accounts still need a live backend session for
            // cross-device KYC/admin operations. Keep the existing behavior.
            val backendResult = connectBackendSession(localUser, password)
            if (backendResult.isFailure && localUser.role == "ADMIN") {
                return Result.failure(
                    backendResult.exceptionOrNull()
                        ?: SecurityException("Secure admin backend session is required")
                )
            }
        ...
        // Fresh-device path: the Room database has no local account, so
        // authenticate the existing account directly against the backend.
        // This does NOT register a new account from the login screen.
        ...
        val restoredUser = UserEntity(
            ...
            normalizedEmail = account.email.trim().lowercase(Locale.ROOT),
            passwordHash = hash,
            passwordSalt = salt,
            role = account.role,
        ...
        db.withTransaction {
            ...
            if (restoredUser.role.uppercase(Locale.ROOT) != "ADMIN") {
        ...
        // Keep the backend session returned by the successful login so the
        // ViewModel can immediately synchronize KYC and other backend data.
        backendSession.token = auth.sessionToken
        backendSession.accountId = account.id
        backendSession.localUserId = restoredUser.id
    ...
    suspend fun resetPasswordByEmail(
        ...
        val user = userDao.getUserByNormalizedEmail(email)
            ?: return Result.failure(
                IllegalArgumentException("Account does not exist")
            )
        ...
        return if (updated == 1) {
            ...
            Result.failure(
                IllegalStateException("Password could not be updated")
            )
        }
    }
    ...
    // -------------------------------------------------------------
    // CROSS-DEVICE KYC BACKEND SYNC
    // -------------------------------------------------------------

    private fun mapRemoteKyc(
        remote: BackendKyc,
        localUserId: String = remote.accountId
    ): KycEntity =
        KycEntity(
            userId = localUserId,
            ...
            documentAttached = !remote.frontIdUrl.isNullOrBlank() || !remote.backIdUrl.isNullOrBlank(),
            documentUri = remote.frontIdUrl,
            frontIdUri = remote.frontIdUrl,
            backIdUri = remote.backIdUrl,
            status = remote.status,
            submittedAt = remote.submittedAt,
            reviewedAt = remote.reviewedAt,
            reviewedByAdminId = remote.reviewedByAdminId,
            rejectionReason = remote.rejectionReason
        )

    suspend fun syncKycFromBackend(userId: String): Result<KycEntity?> {
        val auth = backendAuth()
            ?: return Result.failure(
                SecurityException("Backend session is not connected")
            )

        return runCatching {
            /*
             * The backend is authoritative for cross-device KYC.
             * Never use the fresh-device Room placeholder as a fallback.
             */
            val meResponse = backendApi.me(auth)

            if (!meResponse.isSuccessful) {
                throw backendFailure(
                    meResponse.code(),
                    "Could not verify backend account"
                )
            }

            val account = meResponse.body()?.account
                ?: throw IllegalStateException(
                    "Backend returned no authenticated account"
                )

            val sessionAccountId = backendSession.accountId
            if (
                !sessionAccountId.isNullOrBlank() &&
                sessionAccountId != account.id
            ) {
                throw SecurityException("Backend session account mismatch")
            }

            val response = backendApi.myKyc(auth)

            if (!response.isSuccessful) {
                throw backendFailure(
                    response.code(),
                    "Could not load KYC status from backend"
                )
            }

            val remote = response.body()?.kyc
                ?: throw IllegalStateException(
                    "No KYC record exists for this backend account"
                )

            if (remote.accountId != account.id) {
                throw SecurityException("Backend KYC account mismatch")
            }

            /*
             * Cache the backend record under the authenticated backend account
             * ID. This makes VERIFIED/PENDING/REJECTED authoritative locally.
             */
            val local = mapRemoteKyc(remote, account.id)
            kycDao.insertOrUpdateKyc(local)
            local
        }
    }

    /** Six-argument KYC submission used by the current KYC screen/ViewModel. */
    suspend fun submitKyc(
        userId: String,
        fullName: String,
        idType: String,
        ...
        if (fullName.isBlank() || idType.isBlank() || idNumber.isBlank() ||
            frontIdUri.isBlank() || backIdUri.isBlank()) {
            return Result.failure(IllegalArgumentException("All KYC fields and both ID documents are required"))
        }

        val existing = kycDao.getKycForUserSync(userId)
        if (existing?.status == "VERIFIED") {
            return Result.failure(IllegalStateException("KYC is already verified"))
        }
        if (existing?.status == "PENDING") {
            return Result.failure(IllegalStateException("KYC submission is already pending review"))
        }

        val auth = backendAuth()
            ?: return Result.failure(SecurityException("Backend session is required to submit KYC"))

        return runCatching {
            val frontUrl = uploadAttachmentIfNeeded(frontIdUri)
                ?: throw IllegalStateException("Could not upload front ID document")
            val backUrl = uploadAttachmentIfNeeded(backIdUri)
                ?: throw IllegalStateException("Could not upload back ID document")

            val response = backendApi.submitKyc(
                auth,
                KycSubmitRequest(
                    fullName = fullName.trim(),
                    idType = idType.trim(),
            ...
            if (!response.isSuccessful) {
                throw backendFailure(response.code(), "Could not submit KYC")
            }

            val remote = response.body()?.kyc
                ?: throw IllegalStateException("Backend returned no KYC record")
            val local = mapRemoteKyc(remote, userId)
            kycDao.insertOrUpdateKyc(local)
            local
        }
    }

    private suspend fun cacheRemoteKycDocuments(
        remote: BackendKyc,
        localUserId: String
    ): KycEntity {
        var frontLocal: String? = null
        var backLocal: String? = null
        val auth = backendAuth()

        if (auth != null && !remote.frontIdUrl.isNullOrBlank()) {
            runCatching { backendApi.adminKycDocument(auth, remote.accountId, "front") }
                .getOrNull()?.let { response ->
                    if (response.isSuccessful) {
                        response.body()?.let { body ->
                            frontLocal = AttachmentStorage.saveImageBase64(appContext, body.dataBase64, body.contentType, "kyc_front")
                        }
                    }
                }
        }

        if (auth != null && !remote.backIdUrl.isNullOrBlank()) {
            runCatching { backendApi.adminKycDocument(auth, remote.accountId, "back") }
                .getOrNull()?.let { response ->
                    if (response.isSuccessful) {
                        response.body()?.let { body ->
                            backLocal = AttachmentStorage.saveImageBase64(appContext, body.dataBase64, body.contentType, "kyc_back")
                        }
                    }
                }
        }

        val mapped = mapRemoteKyc(remote, localUserId).copy(
            documentUri = frontLocal ?: remote.frontIdUrl,
            frontIdUri = frontLocal ?: remote.frontIdUrl,
            backIdUri = backLocal ?: remote.backIdUrl
        )
        kycDao.insertOrUpdateKyc(mapped)
        return mapped
    }

    private suspend fun refreshPendingKycFromBackend(): Result<List<KycEntity>> {
        val auth = backendAuth()
            ?: return Result.failure(
                SecurityException("Secure admin backend session is required")
            )
        ...
        return runCatching {
            val response = backendApi.adminKycs(auth)
            ...
            val remoteList = response.body()?.kycs.orEmpty()

            // Backend/D1 is the source of truth. Cache each returned record,
            // but return the backend-derived list to the caller as well.
            remoteList.map { remote ->
                cacheRemoteKycDocuments(
                    remote,
                    remote.accountId
                )
            }
    ...
    suspend fun reviewKyc(
        ...
        if (adminUser == null || adminUser.role != "ADMIN") {
            return Result.failure(SecurityException("Unauthorized: Only an authorized admin can review KYC"))
        }
        if (adminUserId == targetUserId) {
            return Result.failure(SecurityException("Security violation: Admin cannot review their own KYC"))
        }

        val auth = backendAuth()
        if (!auth.isNullOrBlank()) {
            return runCatching {
                val response = backendApi.reviewKyc(
                    auth,
                    targetUserId,
                    KycReviewRequest(approve = approve, reason = reason)
                )
                if (!response.isSuccessful) {
                    throw backendFailure(response.code(), "Could not review KYC")
                }
                val remote = response.body()?.kyc
                    ?: throw IllegalStateException("Backend returned no reviewed KYC")
                val local = cacheRemoteKycDocuments(remote, targetUserId)
                kycDao.insertAuditLog(
                    KycAuditLogEntity(
    ...
    fun getPendingKyc(): Flow<List<KycEntity>> =
        kotlinx.coroutines.flow.flow {
            val remote = refreshPendingKycFromBackend()

            if (remote.isSuccess) {
                emit(remote.getOrThrow())
            } else {
                // Preserve existing UI behavior if the backend is temporarily
                // unavailable, but never treat the empty Room cache as the
                // authoritative cross-device KYC source.
                emitAll(kycDao.getPendingKyc())
            }
        }
    ...
    suspend fun pendingKycCount(): Int {
        val remote = refreshPendingKycFromBackend()
        if (remote.isSuccess) {
            return remote.getOrThrow().size
        }
        return kycDao.countPending()
    }
                ...
                val response =
                    backendApi.createAd(
                        ...
                        AdRequest(
                            ...
                            "",
                            "",
                            fiatOrderAmount = amountReal * fiatPriceEtb
                        )
                    )
