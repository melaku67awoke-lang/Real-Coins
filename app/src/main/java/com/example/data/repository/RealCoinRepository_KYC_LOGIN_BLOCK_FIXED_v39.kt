...
class RealCoinRepository(private val db: AppDatabase, context: Context) {
    ...
    suspend fun login(
        ...
        // IMPORTANT LOGIN FIX:
        // A normal account that already exists in the backend may legitimately
        // be logging in on a fresh installation or after app data was cleared.
        // Do NOT reject that account with "not registered on this device".
        // Authenticate against the backend first, then create the local cache.
        if (user == null) {
            ...
            val role = account.role.trim().uppercase(Locale.ROOT)

            if (role != "ADMIN" && role != "USER") {
                return Result.failure(
                    SecurityException(
                        "Backend account has an unsupported role"
                    )
                )
            }

            // Re-create only the local cache. The backend account is NOT
            // registered again, so an existing account is never duplicated.
            val salt = PasswordHasher.generateSalt()
            val hash = PasswordHasher.hashPassword(password, salt)
            val localUserId =
                "RC_USER_${UUID.randomUUID().toString().take(8).uppercase(Locale.ROOT)}"
            ...
            backendSession.token = auth.sessionToken
            backendSession.accountId = account.id
            backendSession.localUserId = localUserId

            runCatching {
                refreshPricingFromBackend()
            }

            // Pull the authoritative KYC status immediately. This prevents
            // a valid VERIFIED account from being incorrectly blocked because
            // the newly-created local cache starts as NOT_SUBMITTED.
            runCatching {
                syncKycFromBackend(localUserId)
            }
        ...
        // The local login is valid, but cross-device KYC/admin operations
        // require a live backend session. Establish it now.
        val backendResult = connectBackendSession(user, password)

        if (backendResult.isFailure && user.role == "ADMIN") {
            return Result.failure(
    ...
    suspend fun syncKycFromBackend(userId: String): Result<KycEntity?> {
        ...
        return runCatching {
            ...
            if (!response.isSuccessful) {
                if (response.code() == 404) return@runCatching getKycForUserSync(userId)
                throw backendFailure(response.code(), "Could not load KYC status")
            }
            val remote = response.body()?.kyc
                ?: return@runCatching getKycForUserSync(userId)
    ...
    suspend fun reviewKyc(
        ...
        // The backend session must belong to the exact admin performing the
        // review. This prevents a stale session left by another account
        // (including a previous test/device session) from being reused for
        // an administrative KYC action.
        val sessionLocalUserId = backendSession.localUserId
        val auth = backendAuth()
        if (auth != null && sessionLocalUserId == adminUserId) {
            return runCatching {
    ...
    suspend fun claimDailyReward(
        ...
        return db.withTransaction {
            ...
            // The (userId, periodKey) unique index is the final database-level
            // replay protection. Treat a concurrent duplicate insert as a
            // normal "already claimed" result instead of allowing a raw
            // SQLite constraint exception to escape.
            try {
                rewardDao.insertRewardClaim(claim)
            } catch (e: Exception) {
                val existingAfterRace = rewardDao.getClaimForPeriod(
                    userId,
                    periodKey
                )
