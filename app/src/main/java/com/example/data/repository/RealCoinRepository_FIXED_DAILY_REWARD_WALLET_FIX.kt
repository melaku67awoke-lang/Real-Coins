...
class RealCoinRepository(private val db: AppDatabase, context: Context) {
    ...
    suspend fun login(
        ...
        // A user restored from the backend on a fresh install, or an older
        // local account created before wallet initialization was enforced,
        // may not have a Room wallet yet. Daily rewards, deposits,
        // withdrawals, P2P, and the dashboard all depend on this row.
        // Create it once if it is missing; never overwrite an existing wallet.
        if (backendRole != "ADMIN" && walletDao.getWalletSync(user.id) == null) {
            walletDao.insertWallet(
                WalletEntity(
                    userId = user.id,
                    realBalance = 0.0,
    ...
    suspend fun claimDailyReward(
        ...
        return db.withTransaction {
            ...
            val freeAvailable =
                state?.lastFreeSpinAt == null ||
                    now >=
                    state.lastFreeSpinAt + SPIN_COOLDOWN_MS
