...
class RealCoinRepository(private val db: AppDatabase, context: Context) {
    ...
    suspend fun login(
        ...
        // If local data was removed by uninstall/clear-data, authenticate
        // against the persistent backend. Only an existing backend ADMIN
        // may be restored automatically; normal users must use local
        // registration/login flow rather than being silently recreated.
        if (user == null) {
            ...
            if (account.role != "ADMIN") {
                return Result.failure(
                    IllegalArgumentException(
                        "Account is not registered on this device. Please use the normal registration flow."
                    )
                )
            }
            ...
            backendSession.token = auth.sessionToken
            backendSession.accountId = account.id
            backendSession.localUserId = restoredAdmin.id

            runCatching {
                refreshPricingFromBackend()
            }

            return Result.success(restoredAdmin)
        }
