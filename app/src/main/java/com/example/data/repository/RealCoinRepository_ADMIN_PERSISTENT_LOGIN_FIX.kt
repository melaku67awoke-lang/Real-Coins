...
class RealCoinRepository(private val db: AppDatabase, context: Context) {
    ...
    suspend fun login(
         ...
         * ADMIN PERSISTENCE:
         *
         * Room is erased when the app is uninstalled, but the real
         * admin account lives in the Cloudflare/D1 backend. If the
         * local admin record is missing after reinstall, authenticate
         * against the backend and restore a local ADMIN cache.
         *
         * This is deliberately limited to an account whose backend
         * role is ADMIN. A normal backend USER is never recreated
         * through this fallback, so ordinary registration behavior
         * remains unchanged.
         */
        val backendLogin =
            runCatching {
        ...
        if (
            backendLogin?.isSuccessful == true &&
            backendLogin.body()?.sessionToken != null
        ) {
            ...
            if (account != null && account.role == "ADMIN") {
                ...
                db.withTransaction {
                    ...
                    /*
                     * Admin accounts do not submit KYC. Keep a local
                     * non-pending record only so generic local DAO
                     * lookups do not treat the admin as a KYC case.
                     */
