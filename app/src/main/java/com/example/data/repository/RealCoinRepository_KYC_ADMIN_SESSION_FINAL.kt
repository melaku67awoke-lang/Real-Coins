...
class RealCoinRepository(private val db: AppDatabase, context: Context) {
    ...
    suspend fun connectBackendSession(
        ...
        return runCatching {
            ...
            /*
             * ADMIN SECURITY:
             * An existing local ADMIN must already exist as ADMIN on the
             * backend. Never auto-register a missing ADMIN as a USER.
             */
            val auth = if (
                login.isSuccessful &&
                ...
                throw backendFailure(
                    login.code(),
                    if (user.role == "ADMIN") {
                        "Secure admin backend login failed"
                    } else {
    ...
    suspend fun login(
        ...
        if (localUser != null) {
            ...
            /*
             * Every successful login must establish the shared backend
             * session. This is required for cross-device KYC and for
             * the ADMIN control panel.
             *
             * ADMIN is never allowed to continue with only a local
             * Room session because that would make the admin panel
             * appear empty after reinstall/data clear.
             */
            val backendResult =
            ...
            if (backendResult.isFailure) {
                return Result.failure(
                    backendResult.exceptionOrNull()
                        ?: SecurityException(
                            "Secure backend session is required"
                        )
                )
            }
         ...
         * ADMIN PERSISTENCE:
         *
         * Room is erased on uninstall/data clear, but the real ADMIN
         * account remains in Cloudflare/D1. Restore the local ADMIN
         * cache only after the backend authenticates the account and
         * explicitly reports role=ADMIN.
         *
         * A normal backend USER is never recreated through this path.
         */
        val backendLogin =
            runCatching {
                backendApi.login(
        ...
        val restoredAdmin =
            UserEntity(
                ...
                normalizedEmail = account.email.lowercase(Locale.ROOT),
                passwordHash = hash,
                passwordSalt = salt,
                role = "ADMIN",
        ...
        db.withTransaction {
            ...
            if (walletDao.getWalletSync(restoredAdmin.id) == null) {
    ...
    private suspend fun refreshPendingKycFromBackend(): Result<List<KycEntity>> {
        val auth = backendAuth()
            ?: return Result.failure(SecurityException("Secure admin backend session is required"))

        return runCatching {
            val response = backendApi.adminKycs(auth)
            if (!response.isSuccessful) {
                throw backendFailure(response.code(), "Could not load pending KYC")
            }
            val remoteList = response.body()?.kycs.orEmpty()
            remoteList.map { cacheRemoteKycDocuments(it, it.accountId) }
        }
    }
    ...
    fun getPendingKyc(): Flow<List<KycEntity>> =
        kotlinx.coroutines.flow.flow {
            // Admin KYC must come from the shared Cloudflare/D1 backend first.
            // Room is only the local cache used by the UI after synchronization.
            runCatching { refreshPendingKycFromBackend() }
