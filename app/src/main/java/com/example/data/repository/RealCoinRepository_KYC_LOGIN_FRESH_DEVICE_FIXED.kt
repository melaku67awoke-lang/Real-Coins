...
class RealCoinRepository(private val db: AppDatabase, context: Context) {
    ...
    suspend fun login(
        ...
        /* Fresh-device synchronization: authenticate an existing backend
         * account directly. Never register from the login screen. */
        val backendLogin = runCatching {
            backendApi.login(
                AuthRequest(email = normalized, password = password)
            )
        }.getOrElse { error ->
            return Result.failure(
                IllegalStateException(error.message ?: "Could not connect to the backend")
            )
        }

        if (!backendLogin.isSuccessful ||
            backendLogin.body()?.sessionToken.isNullOrBlank()) {
            return Result.failure(
                IllegalArgumentException(
                    when (backendLogin.code()) {
                        401 -> "Invalid email or password"
                        404 -> "Account does not exist. Please register first."
                        else -> "Account could not be authenticated (HTTP ${backendLogin.code()})"
                    }
                )
            )
        }
        ...
        val auth = backendLogin.body()
            ?: return Result.failure(IllegalStateException("Backend returned no authentication response"))
        val account = auth.account
            ?: return Result.failure(IllegalStateException("Backend returned no account"))

        val backendEmail = account.email?.trim()?.lowercase(Locale.ROOT)?.takeIf { it.isNotBlank() } ?: normalized
        val backendUsername = account.username?.trim()?.takeIf { it.isNotBlank() } ?: query
        val backendRole = account.role?.trim()?.uppercase(Locale.ROOT) ?: "USER"
        val localUserId = "RC_BACKEND_${account.id}"

        val existingByBackendId = userDao.getUserById(localUserId)
        val existingByEmail = userDao.getUserByNormalizedEmail(backendEmail)
        val user = existingByBackendId ?: existingByEmail ?: run {
            val salt = PasswordHasher.generateSalt()
        ...
        return runCatching {
            db.withTransaction {
                if (existingByBackendId == null && existingByEmail == null) {
            ...
            onFailure = { error -> Result.failure(IllegalStateException(error.message ?: "Could not synchronize account to this device")) }
        )
    }
