...
class RealCoinRepository(private val db: AppDatabase, context: Context) {
    ...
    suspend fun login(
        ...
        // existing local authentication path for normal users and for the
        // admin account created during a fresh installation.
        val localUser =
        ...
        // A fresh installation has an empty Room database. The existing
        // administrator account is stored in the Cloudflare backend and
        // must never be forced through USER registration/KYC.
        //
        // Only an email login may use this recovery path, and only a
        // backend account whose role is ADMIN may be restored locally.
        // This prevents an unverified USER backend account from bypassing
        // the normal local KYC gate.
        ...
        if (backendLogin == null || !backendLogin.isSuccessful) {
            return Result.failure(
                IllegalArgumentException(
                    "Account does not exist. Please register first."
                )
        ...
        if (account == null || sessionToken.isNullOrBlank()) {
            return Result.failure(
                IllegalArgumentException(
                    "Account does not exist. Please register first."
                )
        ...
        val restoredAdmin = UserEntity(
            ...
            normalizedEmail =
                account.email.trim().lowercase(Locale.ROOT),
            passwordHash = hash,
            passwordSalt = salt,
            role = "ADMIN",
        ...
        db.withTransaction {
            ...
            val existingWallet =
                walletDao.getWalletSync(restoredAdmin.id)
        ...
        // Keep the authenticated backend session immediately.
        backendSession.token = sessionToken
        backendSession.accountId = account.id
        backendSession.localUserId = restoredAdmin.id

        // A restored admin has no local pricing settings after a fresh install.
        // Pull the authoritative backend price immediately after the backend
        // session is established so reinstall/login cannot fall back to 0.0027.
        runCatching {
            refreshPricingFromBackend()
        }
    ...
    private fun mapRemoteKyc(
        ...
        KycEntity(
            ...
            documentAttached =
                !remote.frontIdUrl.isNullOrBlank() ||
                    !remote.backIdUrl.isNullOrBlank(),
    ...
    suspend fun syncKycFromBackend(
        userId: String
    ): Result<KycEntity?> {
        val auth = backendAuth()
            ?: return Result.failure(
    ...
    /** UI/ViewModel-compatible six-argument KYC submission. */
    suspend fun submitKyc(
        userId: String,
        fullName: String,
        idType: String,
        ...
        if (fullName.isBlank() || idType.isBlank() || idNumber.isBlank() ||
            ...
            return Result.failure(
                IllegalArgumentException(
                    "All KYC fields and both ID documents are required"
                )
            )
        }
        ...
        val auth = backendAuth()
            ?: return Result.failure(
                SecurityException("Backend session is required to submit KYC")
            )
    ...
    private suspend fun fetchAdminKycRemote(): Result<AdminKycResponse> {
        val auth = backendAuth()
            ?: return Result.failure(
        ...
        return runCatching {
            ...
            response.body()
                ?: throw IllegalStateException("Backend returned no KYC data")
        }
    }
        ...
        if (auth != null && !remote.frontIdUrl.isNullOrBlank()) {
            runCatching {
                ...
                if (response.isSuccessful) {
                    ...
                    if (body != null) {
                        frontLocal = AttachmentStorage.saveImageBase64(
                            appContext,
                            body.dataBase64,
                            body.contentType,
                            "kyc_front"
        ...
        if (auth != null && !remote.backIdUrl.isNullOrBlank()) {
            runCatching {
                ...
                if (response.isSuccessful) {
                    val body = response.body()
                    if (body != null) {
                        backLocal = AttachmentStorage.saveImageBase64(
                            appContext,
                            body.dataBase64,
                            body.contentType,
                            "kyc_back"
    ...
    suspend fun reviewKyc(
        ...
        val localTarget = kycDao.getKycForUserSync(targetUserId)

        // When an authenticated backend session exists, the backend is the
        // authoritative KYC review source. This is required for an admin on
        // one phone to approve/reject a user whose account originated on
        // another phone. The local record is only the cached UI copy.
        if (backendAuth() != null) {
            return runCatching {
                ...
                cacheRemoteKycDocuments(remote, targetUserId)
                ...
                kycDao.insertAuditLog(
                    KycAuditLogEntity(
                        ...
                        notes = reason
                            ?: if (approve) "Approved by compliance" else "Rejected",
                        timestamp = System.currentTimeMillis()
                    )
                )

                mapRemoteKyc(remote, targetUserId)
            }
        }

        // Preserve the existing local/test authorization path for records
        // that already exist on this device.
        return db.withTransaction {
            val targetKyc =
                kycDao.getKycForUserSync(targetUserId)
                    ?: return@withTransaction Result.failure(
                        IllegalStateException("Target KYC not found")
                    )
    ...
    fun getPendingKyc(): Flow<List<KycEntity>> =
        kotlinx.coroutines.flow.flow {
            val remoteResult = fetchAdminKycRemote()

            if (remoteResult.isSuccess) {
                val remote = remoteResult.getOrThrow()
                val mapped = remote.kycs
                    .filter { it.status.equals("PENDING", ignoreCase = true) }
                    .map { cacheRemoteKycDocuments(it, it.accountId) }
                emit(mapped)
            } else {
