...
class RealCoinRepository(private val db: AppDatabase) {
    ...
    suspend fun getUsdToEtbRate(): Double =
        appSettingsDao.get("USD_TO_ETB")?.value ?: 187.0
    ...
    suspend fun updatePricing(adminUserId: String, realCoinUsdPrice: Double, usdToEtbRate: Double): Result<Unit> {
        if (!realCoinUsdPrice.isFinite() || realCoinUsdPrice <= 0.0 ||
            !usdToEtbRate.isFinite() || usdToEtbRate <= 0.0) {
            return Result.failure(IllegalArgumentException("Price and exchange rate must be greater than zero"))
        }
        return db.withTransaction {
            val admin = userDao.getUserById(adminUserId)
                ?: return@withTransaction Result.failure(SecurityException("Admin not found"))
            if (admin.role != "ADMIN") {
                return@withTransaction Result.failure(SecurityException("Only an authorized admin can change pricing"))
            }
            appSettingsDao.upsert(AppSettingEntity("REAL_COIN_USD_VALUE", realCoinUsdPrice))
            appSettingsDao.upsert(AppSettingEntity("USD_TO_ETB", usdToEtbRate))
            Result.success(Unit)
        }
    }
    ...
    fun getActiveP2PAdsForUser(userId: String): Flow<List<P2PAdEntity>> =
        p2pDao.getActiveAdsForUser(userId)

    suspend fun deleteOwnP2PAd(userId: String, adId: String): Result<Unit> {
        return db.withTransaction {
            val ad = p2pDao.getAdById(adId)
                ?: return@withTransaction Result.failure(IllegalStateException("Advertisement not found"))

            if (ad.sellerId != userId) {
                return@withTransaction Result.failure(SecurityException("You can delete only your own advertisement"))
            }
            if (!ad.isActive) {
                return@withTransaction Result.failure(IllegalStateException("Advertisement is already inactive"))
            }
            // Never release a listing reservation while an order is still open.
            // Otherwise the seller could delete the ad, regain the REAL, and the
            // existing order could later release the same REAL a second time.
            if (p2pDao.countOpenOrdersForAd(adId) > 0) {
                return@withTransaction Result.failure(IllegalStateException("This advertisement has an active order and cannot be deleted"))
            }

            if (ad.type != "SELL") {
                // BUY listings do not reserve REAL, so only deactivate them.
            ...
            val wallet = walletDao.getWalletSync(userId)
                ?: return@withTransaction Result.failure(IllegalStateException("Wallet not found"))
            val releaseAmount = ad.cryptoAmount.coerceAtLeast(0.0)
            if (wallet.realLockedBalance + 1e-9 < releaseAmount) {
                return@withTransaction Result.failure(IllegalStateException("Reserved REAL balance is inconsistent; deletion was not completed"))
            }

            // A listing reservation is released atomically with deactivation.
            p2pDao.updateAd(ad.copy(isActive = false))
            walletDao.updateWallet(
                wallet.copy(
                    realLockedBalance = (wallet.realLockedBalance - releaseAmount).coerceAtLeast(0.0),
                    updatedAt = System.currentTimeMillis()
                )
            )
            ledgerDao.insertLedgerEntry(
                LedgerEntryEntity(
                    id = UUID.randomUUID().toString(),
                    userId = userId,
                    type = "P2P_AD_RESERVATION_RELEASE",
                    currency = "REAL",
                    amount = releaseAmount,
                    balanceBefore = wallet.realBalance,
                    balanceAfter = wallet.realBalance,
                    referenceId = adId,
                    notes = "Released REAL reserved for deleted P2P sell advertisement: $releaseAmount RC",
                    timestamp = System.currentTimeMillis()
                )
            )
            Result.success(Unit)
    ...
    suspend fun createP2PSellAd(
        ...
        return db.withTransaction {
            ...
            val kyc = kycDao.getKycForUserSync(sellerId)
            if (kyc == null || kyc.status != "VERIFIED" || kyc.fullName.isBlank()) {
                return@withTransaction Result.failure(IllegalStateException("Approved KYC is required to create a P2P advertisement"))
            }
            val ad = P2PAdEntity(
                id = adId,
                sellerId = sellerId,
                sellerName = kyc.fullName.trim(),
                type = "SELL",
            ...
            ledgerDao.insertLedgerEntry(
                LedgerEntryEntity(
                    id = UUID.randomUUID().toString(),
                    userId = sellerId,
                    type = "P2P_AD_RESERVATION",
                    currency = "REAL",
                    amount = -amountReal,
                    balanceBefore = wallet.realBalance,
                    balanceAfter = wallet.realBalance,
                    referenceId = adId,
                    notes = "REAL reserved for active P2P sell advertisement: $amountReal RC",
                    timestamp = System.currentTimeMillis()
                )
            )
    ...
    suspend fun startP2PTrade(buyerId: String, adId: String): Result<P2POrderEntity> {
        return db.withTransaction {
            ...
            // One advertisement may back only one open order at a time. This
            // prevents duplicate orders and duplicate escrow releases.
            if (p2pDao.countOpenOrdersForAd(adId) > 0) {
                return@withTransaction Result.failure(IllegalStateException("This advertisement already has an active order"))
            }
            ...
            val sellerKyc = kycDao.getKycForUserSync(ad.sellerId)
            val buyerKyc = kycDao.getKycForUserSync(buyerId)
            val order = P2POrderEntity(
                id = orderId,
                adId = ad.id,
                sellerId = ad.sellerId,
                sellerName = sellerKyc?.fullName?.trim().takeUnless { it.isNullOrBlank() } ?: ad.sellerName,
                buyerId = buyerId,
                buyerName = buyerKyc?.fullName?.trim().takeUnless { it.isNullOrBlank() } ?: buyer.username,
                ...
                createdAt = System.currentTimeMillis(),
                expiresAt = System.currentTimeMillis() + 25L * 60L * 1000L
            )
    ...
    suspend fun expireP2POrderIfUnpaid(orderId: String): Result<P2POrderEntity> {
        return db.withTransaction {
            ...
            if (order.status != "ESCROW_LOCKED") return@withTransaction Result.success(order)
            if (System.currentTimeMillis() < order.expiresAt) {
                return@withTransaction Result.failure(IllegalStateException("Order has not expired yet"))
            }
            val ad = p2pDao.getAdById(order.adId)
            if (ad != null && ad.sellerId == order.sellerId && !ad.isActive) {
                p2pDao.updateAd(ad.copy(isActive = true))
            }
            val expired = order.copy(status = "CANCELLED", completedAt = System.currentTimeMillis())
            p2pDao.updateOrder(expired)
