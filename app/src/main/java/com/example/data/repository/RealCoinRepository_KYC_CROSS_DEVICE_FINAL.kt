...
class RealCoinRepository(private val db: AppDatabase, context: Context) {
    ...
    private fun remoteAccountId(remote: BackendKyc): String =
        remote.accountId?.takeIf { it.isNotBlank() }
            ?: remote.userId?.takeIf { it.isNotBlank() }
            ?: throw IllegalStateException("Backend KYC record has no account ID")
    ...
    private fun mapRemoteKyc(
        remote: BackendKyc,
        localUserId: String = remoteAccountId(remote)
    ): KycEntity =
        KycEntity(
            userId = localUserId,
    ...
    suspend fun syncKycFromBackend(userId: String): Result<KycEntity?> {
        ...
        return runCatching {
            var response = backendApi.myKyc(auth)
            if (!response.isSuccessful && response.code() in listOf(404, 405)) {
                response = backendApi.myKycLegacy(auth)
            }
            if (!response.isSuccessful) {
    ...
    private suspend fun cacheRemoteKycDocuments(
        ...
        if (auth != null && !remote.frontIdUrl.isNullOrBlank()) {
            runCatching { backendApi.adminKycDocument(auth, remoteAccountId(remote), "front") }
        ...
        if (auth != null && !remote.backIdUrl.isNullOrBlank()) {
            runCatching { backendApi.adminKycDocument(auth, remoteAccountId(remote), "back") }
                .getOrNull()?.let { response ->
                    if (response.isSuccessful) {
                        response.body()?.let { body ->
    ...
    private suspend fun refreshPendingKycFromBackend(): Result<List<KycEntity>> {
        ...
        return runCatching {
            var response = backendApi.adminKycs(auth)
            if (!response.isSuccessful && response.code() in listOf(404, 405)) {
                response = backendApi.adminKycsLegacy(auth)
            }
        ...
        if (!auth.isNullOrBlank()) {
            val remote = runCatching {
                ...
                if (!response.isSuccessful) {
                    throw backendFailure(
                        response.code(),
                        "Could not load KYC statistics"
                    )
                }
                response.body()?.verifiedCount ?: 0
            }.getOrNull()
