...
class RealCoinRepository(private val db: AppDatabase, context: Context) {
    ...
    suspend fun getDailyRewardNextAt(
        userId: String
    ): Long {
        val lastClaim =
            rewardDao.getClaimsForUser(userId)
                .maxByOrNull { it.claimTimestamp }
                ?.claimTimestamp
                ?: return 0L

        return lastClaim + SPIN_COOLDOWN_MS
    }
    ...
    suspend fun claimDailyReward(
        ...
        return db.withTransaction {
            ...
            if (lastClaim != null &&
                now < lastClaim.claimTimestamp + SPIN_COOLDOWN_MS
            ) {
                return@withTransaction Result.failure(
                    IllegalStateException(
                        "Daily reward is not ready yet. Please wait 24 hours between claims."
                    )
                )
            }

            val amount = rewardStatus.level.dailyRewardReal
            ...
            rewardDao.insertRewardClaim(
                RewardClaimEntity(
                    ...
                    amountReal = amount,
                    claimTimestamp = now,
                    periodKey = "24H_${now / SPIN_COOLDOWN_MS}"
                )
            )
            ...
            ledgerDao.insertLedgerEntry(
                LedgerEntryEntity(
                    ...
                    before,
                    after,
                    "DAILY_${UUID.randomUUID().toString().take(8)}",
                    "Daily reward - ${rewardStatus.level.name}",
                    now
                )
            )
