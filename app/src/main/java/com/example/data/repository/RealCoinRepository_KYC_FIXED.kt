...
class RealCoinRepository(private val db: AppDatabase, context: Context) {
    ...
    private fun mapRemoteKyc(
        remote: BackendKyc,
        localUserId: String = remote.accountId.ifBlank { remote.userId.orEmpty() }
    ): KycEntity =
        KycEntity(
            userId = localUserId,
    ...
    private suspend fun cacheRemoteKycDocuments(
        ...
        val remoteAccountId = remote.accountId.ifBlank { remote.userId.orEmpty() }
        var frontLocal: String? = null
        var backLocal: String? = null
        val auth = backendAuth()

        if (auth != null && !remote.frontIdUrl.isNullOrBlank()) {
            runCatching { backendApi.adminKycDocument(auth, remoteAccountId, "front") }
        ...
        if (auth != null && !remote.backIdUrl.isNullOrBlank()) {
            runCatching { backendApi.adminKycDocument(auth, remoteAccountId, "back") }
                .getOrNull()?.let { response ->
                    if (response.isSuccessful) {
                        response.body()?.let { body ->
        ...
        return runCatching {
            ...
            remoteList.map { remote ->
                cacheRemoteKycDocuments(
                    remote,
                    remote.accountId.ifBlank { remote.userId.orEmpty() }
                )
            }
        }
    }
