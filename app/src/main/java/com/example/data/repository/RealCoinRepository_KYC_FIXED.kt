...
class RealCoinRepository(private val db: AppDatabase, context: Context) {
        ...
        return runCatching {
            ...
            val auth = if (
                ...
                when {
                    ...
                    registration.code() == 409 -> {
                        // The backend account already exists. This can happen
                        // when a previous registration succeeded on the
                        // backend but the Android app did not finish syncing.
                        // Re-login instead of reporting a false registration
                        // failure.
                        val existingLogin = backendApi.login(
                            AuthRequest(
                                email = user.normalizedEmail,
                                password = password
                            )
                        ...
                        if (
                            !existingLogin.isSuccessful ||
                            existingLogin.body()?.sessionToken.isNullOrBlank()
                        ) {
                            throw backendFailure(
                                existingLogin.code(),
                                "Backend account exists, but login failed"
                            )
                        }
    ...
    /**
     * Logs a user in against the local Room account and, when necessary,
     * synchronizes an ADMIN role from the backend.
     *
     * The backend remains the authority for ADMIN status. This is important
     * because /v1/admin/bootstrap promotes the backend account, but the old
     * implementation never copied that role into the local Room database.
     *
     * If the local account is missing and the login input is an email, the
     * backend is checked first. A successfully authenticated backend account
     * is then created locally with the same password, username, email, and
     * backend role. This fixes the first-admin-login case without hard-coding
     * an administrator or exposing ADMIN_BOOTSTRAP_SECRET in the Android app.
     */
    suspend fun login(
        ...
        if (user == null) {
            // Backend authentication can only be attempted here when the
            // supplied identifier is an email. The backend login endpoint
            // expects an email, while local Room login also accepts usernames.
            ...
            val backendLogin = runCatching {
                ...
                return Result.failure(
                    IllegalStateException(
                        "Unable to verify the account with the backend: ${error.message ?: "network error"}"
                    )
                )
            }
            ...
            if (
                ...
                backendLogin.body()?.account == null ||
                backendLogin.body()?.sessionToken.isNullOrBlank()
            ) {
                return if (backendLogin.code() == 401 || backendLogin.code() == 404) {
                    Result.failure(
                        IllegalArgumentException(
                            "Account does not exist. Please register first."
                        )
            ...
            val localPasswordSalt = PasswordHasher.generateSalt()
            val localPasswordHash =
            ...
            val localUser = UserEntity(
                id = "RC_USER_${UUID.randomUUID().toString().take(8).uppercase(Locale.ROOT)}",
                username = account.username,
                normalizedEmail = account.email.trim().lowercase(Locale.ROOT),
                passwordHash = localPasswordHash,
                passwordSalt = localPasswordSalt,
                role = account.role.trim().uppercase(Locale.ROOT).takeIf { it == "ADMIN" } ?: "USER",
            ...
            try {
                db.withTransaction {
                    // Re-check inside the transaction in case another login
                    // created the same local account concurrently.
                    val existing =
                        userDao.getUserByNormalizedEmail(localUser.normalizedEmail)
                ...
                return Result.failure(
                    IllegalStateException(
                        "Backend login succeeded, but the local account could not be created: ${error.message ?: "database error"}"
                    )
                )
            }
            ...
            user =
                userDao.getUserByNormalizedEmail(
                    localUser.normalizedEmail
                ) ?: localUser

            // Keep the backend session produced by this login attempt so the
            // ViewModel can continue directly into the normal session setup.
            backendSession.token = auth.sessionToken
            backendSession.accountId = account.id
            backendSession.localUserId = user.id
        ...
        // The backend is the authority for ADMIN status. Check it before the
        // ViewModel performs its KYC decision so a backend-promoted admin is
        // recognized locally, and a backend-demoted admin cannot retain stale
        // local ADMIN privileges.
        val localRole =
            localUser.role.trim().uppercase(Locale.ROOT)

        if (localRole == "ADMIN" || normalized.contains("@")) {
            val backendLogin = runCatching {
                backendApi.login(
                    AuthRequest(
                ...
                if (localRole == "ADMIN") {
                    return Result.failure(
                        IllegalStateException(
                            "Unable to verify the administrator account with the backend: ${error.message ?: "network error"}"
                        )
                    )
                }
                null
            ...
            if (backendLogin?.isSuccessful == true) {
                val backendAuth = backendLogin.body()
                val backendAccount = backendAuth?.account
                val backendRole =
                    backendAccount?.role
                        ?.trim()
                        ?.uppercase(Locale.ROOT)
                ...
                if (
                    backendAccount != null &&
                    !backendAuth.sessionToken.isNullOrBlank()
                ) {
                    if (backendRole == "ADMIN") {
                        if (localRole != "ADMIN") {
                            userDao.updateRole(localUser.id, "ADMIN")
                        }

                        backendSession.token = backendAuth.sessionToken
                        backendSession.accountId = backendAccount.id
                        backendSession.localUserId = localUser.id

                        user = localUser.copy(role = "ADMIN")
                    } else if (localRole == "ADMIN") {
                        // The account is no longer an admin on the backend.
                        // Remove stale local privilege before returning to the
                        // ViewModel, which will then apply the normal-user KYC
                        // rules.
                        userDao.updateRole(localUser.id, "USER")
                        user = localUser.copy(role = "USER")
                    }
                } else if (localRole == "ADMIN") {
                    return Result.failure(
                        backendFailure(
                            backendLogin.code(),
                            "Backend administrator account could not be verified"
                        )
                    )
                }
            } else if (localRole == "ADMIN") {
    ...
    suspend fun submitKyc(
        ...
        val cleanIdType = idType.trim()
        val cleanIdNumber = idNumber.trim()
        val cleanFrontIdUri = frontIdUri.trim()
        val cleanBackIdUri = backIdUri.trim()

        val allowedIdTypes = setOf(
            "National ID",
            "Passport",
            "Driving License"
        )
        ...
        if (cleanIdType !in allowedIdTypes) {
            return Result.failure(
                IllegalArgumentException(
        ...
        if (cleanFrontIdUri.isBlank()) {
            return Result.failure(
                IllegalArgumentException(
                    "Front of ID is required. Select it from your file or gallery."
                )
            )
        }

        if (cleanBackIdUri.isBlank()) {
            return Result.failure(
                IllegalArgumentException(
                    "Back of ID is required. Select it from your file or gallery."
                )
            )
        }
        ...
        return db.withTransaction {
            ...
            val updatedKyc = KycEntity(
                ...
                documentUri = cleanFrontIdUri,
                frontIdUri = cleanFrontIdUri,
                backIdUri = cleanBackIdUri,
                status = "PENDING",
