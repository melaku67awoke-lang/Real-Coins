    ...
    suspend fun claimDailyReward(
        ...
        return db.withTransaction {
            ...
            val rewardStatus =
                getRewardStatus(userId)
                    ?: return@withTransaction Result.failure(
                        IllegalStateException("Reward status unavailable")
                    )
            ...
            if (latestClaim != null) {
                val nextClaimAt =
                    latestClaim.claimTimestamp + DAILY_REWARD_COOLDOWN_MS

                if (now < nextClaimAt) {
                    return@withTransaction Result.failure(
                        IllegalStateException(
                            "Daily reward is not ready yet. Try again after 24 hours."
                        )
                    )
                }
            }
            ...
            val after = before + rewardAmount
            val referenceId =
                "DAILY_REWARD_${UUID.randomUUID()}"
            ...
            ledgerDao.insertLedgerEntry(
                LedgerEntryEntity(
                    ...
                    notes = "Daily reward for ${rewardStatus.level.name} level",
                    timestamp = now
                )
            )
