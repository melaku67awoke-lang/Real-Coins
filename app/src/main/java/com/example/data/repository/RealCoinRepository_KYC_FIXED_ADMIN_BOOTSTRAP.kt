...
class RealCoinRepository(private val db: AppDatabase, context: Context) {
    ...
    suspend fun login(
        ...
        // First use the local Room account. This preserves the normal
        // "register first" behavior for accounts that do not exist locally.
        val localUser =
        ...
        /*
         * ADMIN BOOTSTRAP / LOCAL SYNC
         *
         * An administrator is authorized by the backend. A fresh Android
         * installation has an empty Room database, so a backend ADMIN must
         * not be forced through normal user registration just to create a
         * local copy.
         *
         * IMPORTANT: We only create a local account when the backend login
         * succeeds AND the backend explicitly says role == ADMIN. A normal
         * backend USER is never imported here, so an unregistered normal
         * user still receives the existing "Please register first" message.
         */
        val adminBootstrap = runCatching {
            backendApi.login(
                AuthRequest(
                    email = normalized,
                    password = password
        ...
        val backendAccount =
            if (adminBootstrap?.isSuccessful == true) {
                adminBootstrap.body()?.account
            } else {
                null
            }

        if (backendAccount?.role?.uppercase(Locale.ROOT) == "ADMIN") {
            val salt = PasswordHasher.generateSalt()
            val hash = PasswordHasher.hashPassword(
                password,
                salt
            ...
            return runCatching {
                db.withTransaction {
                    ...
                    // Admins do not require KYC, but keeping a wallet row
                    // makes the local account compatible with the existing
                ...
                // If another process already synchronized the same admin,
                // use that local row after re-reading it.
                val existing =
                    userDao.getUserByNormalizedEmail(
