...
class RealCoinRepository(private val db: AppDatabase, context: Context) {
    ...
    suspend fun connectBackendSession(
        ...
        /*
         * Login already performs the authoritative backend login and stores
         * the returned session. Do not immediately call /login a second time.
         * A second request can trigger backend rate limiting (HTTP 429).
         */
        if (
            backendSession.authHeader() != null &&
    ...
    suspend fun login(
        ...
        if (localUser == null) {
            /*
             * Fresh-install ADMIN bootstrap:
             * only a backend account that explicitly reports role ADMIN
             * may be imported into the local Room database. Normal backend
             * users are never imported, so an unknown normal account keeps
             * the existing "Please register first" behavior.
             */
            ...
            /*
             * Keep the successful backend session so MainActivity does not
             * immediately perform a second login request. This is especially
             * important for rate-limited backends that return HTTP 429 after
             * repeated authentication calls.
             */
            if (
                backendAuthResponse?.sessionToken?.isNotBlank() == true &&
                backendAuthResponse.account != null
            ) {
                backendSession.token = backendAuthResponse.sessionToken
                backendSession.accountId = backendAuthResponse.account.id
                backendSession.localUserId = localUser?.id
            }

            val backendAccount = backendAuthResponse?.account

            if (backendLogin?.isSuccessful == false && backendLogin.code() == 429) {
                return Result.failure(
                    IllegalStateException(
                        "Backend login temporarily rate-limited (HTTP 429). Please wait and try again."
                    )
                )
            }
            ...
            if (backendLogin?.isSuccessful == true &&
                ...
                return Result.failure(
                    IllegalStateException(
                        "Backend login succeeded but returned no account."
                    )
                )
            }

            if (backendAccount?.role?.equals("ADMIN", ignoreCase = true) == true) {
                val salt = PasswordHasher.generateSalt()
                val hash = PasswordHasher.hashPassword(
                    password,
                    salt
                ...
                val adminUser = UserEntity(
                    ...
                    normalizedEmail = backendAccount.email
                        .trim()
                        .lowercase(Locale.ROOT),
                    passwordHash = hash,
                    passwordSalt = salt,
                    role = "ADMIN",
                    referralCode = backendAccount.referralCode,
                    createdAt = System.currentTimeMillis()
                )
                ...
                return runCatching {
                    ...
                    /*
                     * The backend login above already authenticated this
                     * administrator and returned a session token. The
                     * previous code stored localUserId before the local admin
                     * account existed, leaving it null. MainActivity then
                     * called /login a second time immediately and could hit
                     * the backend's login rate limit (HTTP 429).
                     *
                     * Bind the existing authenticated backend session to the
                     * newly-created local admin account so no second login
                     * request is made.
                     */
                    backendSession.localUserId = adminUser.id
                    ...
                    if (existing?.role == "ADMIN") {
                        ...
                        Result.failure(
                            IllegalStateException(
                                error.message
                                    ?: "Could not create local admin account"
                            )
                        )
                    }
                }
        ...
        /*
         * Existing local account may have been created as USER before the
         * server-side one-time admin bootstrap promoted the same email.
         * Re-check the authoritative backend role after local credentials
         * are verified. If the backend explicitly reports ADMIN, synchronize
         * the local role to ADMIN. If the backend is unavailable or reports
         * USER, keep the existing local role unchanged.
         */
        val backendLogin = runCatching {
            backendApi.login(
                AuthRequest(
        ...
        if (backendAccount?.role?.equals("ADMIN", ignoreCase = true) == true) {
            ...
            if (updated == 1) {
                return Result.success(
                    localUser.copy(role = "ADMIN")
                )
            }

            return Result.failure(
                IllegalStateException(
                    "Could not synchronize administrator role"
                )
            )
        }
    ...
    suspend fun deleteSavedPaymentAccount(
        ...
        paymentAccountDao.getOwned(accountId, userId)
            ?: return Result.failure(
                SecurityException("Payment account not found or not owned by user")
            )

        val hasActiveOrder = p2pDao.getOrdersForUserSync(userId).any { order ->
            order.status == "ESCROW_LOCKED" ||
                order.status == "PAID" ||
        ...
        if (hasActiveOrder) {
            return Result.failure(
                IllegalStateException(
                    "You cannot delete a payment account while you have an active P2P order"
                )
            )
        }
