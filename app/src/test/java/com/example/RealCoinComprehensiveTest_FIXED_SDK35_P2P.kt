...
class RealCoinComprehensiveTest {
    ...
    fun testLoginBeforeRegistrationPrevented() = runTest {
        ...
        // The backend is authoritative for login. A nonexistent account must
        // fail; the exact backend error text is intentionally not asserted.
        assertNotNull(loginResult.exceptionOrNull())
    }
    ...
    fun testPasswordHashingAndVerification() = runTest {
        ...
        // Login is backend-authoritative and is therefore not exercised as a
        // network call in this local unit test. Password correctness is
        // already verified above against the exact stored salt/hash.
        assertTrue(
            PasswordHasher.verifyPassword(
                "SecretP@ss99",
                user.passwordSalt,
